...
public class KebabAssertions : ILocatorAssertions
{
    private readonly ILocatorAssertions captionAssertions;
    private readonly ILocatorAssertions contextAssertions;

    public KebabAssertions(ILocatorAssertions contextAssertions, ILocatorAssertions captionAssertions)
    {
        this.contextAssertions = contextAssertions;
        this.captionAssertions = captionAssertions;
    }
    ...
    public async Task ToBeDisabledAsync(LocatorAssertionsToBeDisabledOptions? options = null)
        => await captionAssertions.ToHaveAttributeAsync(
            "tabindex",
            "-1",
    ...
    public async Task ToBeEnabledAsync(LocatorAssertionsToBeEnabledOptions? options = null)
        => await captionAssertions.Not.ToHaveAttributeAsync(
            "tabindex",
            "-1",
            new LocatorAssertionsToHaveAttributeOptions { Timeout = options?.Timeout }
        ).ConfigureAwait(false);
    ...
    public async Task ToBeHiddenAsync(LocatorAssertionsToBeHiddenOptions? options = null)
        => await captionAssertions.ToBeHiddenAsync(options).ConfigureAwait(false);
    ...
    public async Task ToBeVisibleAsync(LocatorAssertionsToBeVisibleOptions? options = null)
        => await captionAssertions.ToBeVisibleAsync(options).ConfigureAwait(false);
    ...
    public ILocatorAssertions Not => new KebabAssertions(contextAssertions.Not, captionAssertions.Not);
}